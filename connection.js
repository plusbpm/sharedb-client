...
Connection.prototype.handleMessage = function(message) {
  ...
  switch (message.a) {
    ...
    case 'qs':
      ...
      if (query) {
        this.subscribedQueries[queryId] = query;
        query._handleSubscribe(err, message.data, message.extra, message.queryHash);
      }
      return;
    case 'qu':
      delete this.subscribedQueries[message.id];
      return;
    case 'q':
      // Query message. Pass this to the appropriate query objects.
      var queryHash = message.id;
      for (qid in this.queries) {
        var query = this.queries[qid];
        if (query && query.queryHash === queryHash) {
